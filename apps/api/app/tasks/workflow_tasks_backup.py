...
Handles synchronous workflow step execution with robust error handling and comprehensive logging.
"""
...
class WorkflowTask(Task):
    ...
    def __call__(self, *args, **kwargs):
        ...
        logger.info(f"🚀 Starting Celery task {self.name} (ID: {task_id}) with args: {args}")
        
        try:
            if not self.workflow_service:
                self.workflow_service = WorkflowService()
                logger.debug(f"Initialized WorkflowService for task {self.name}")
            
            result = self.run(*args, **kwargs)
            logger.info(f"✅ Successfully completed task {self.name} (ID: {task_id})")
            return result
            
        except Exception as e:
            logger.error(f"❌ Task {self.name} (ID: {task_id}) failed with error: {str(e)}", exc_info=True)
            raise
...
def execute_workflow_step(
    ...
    """
    Execute a single workflow step synchronously (wrapping async operations).
    ...
    """
    logger.info(f"🔧 Starting workflow step execution - Run: {run_id}, Step: {step_id}")
    
    async def _execute_step_async():
        """Inner async function to handle the actual step execution."""
